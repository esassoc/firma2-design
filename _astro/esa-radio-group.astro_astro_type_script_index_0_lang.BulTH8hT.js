import{b as i,i as p,A as r,a as u}from"./lit-element.D8DSg5zn.js";import{t as b}from"./typography.KBHeYOQc.js";import{a as g}from"./a11y.sqk3bMt7.js";const m=i`<svg
  class="error__icon"
  viewBox="0 0 24 24"
  fill="none"
  stroke="currentColor"
...
>
  ...
  />
      ...
      >
        ${this.label?i`<legend class="group-label typography-${f[this.size]}" id="legend">
              ${this.label}${this.required?i`<span class="required" aria-hidden="true">*</span>`:null}
            </legend>`:null}
        ${this.options.map((o,c)=>{const l=this.isSelected(o.value),a=o.disabled??!1,n=`opt-${c}-label`;return i`
            <label
              class="item ${a?"item--disabled":""}"
              @keydown=${h=>this.onKeydown(h,o)}
              @click=${()=>this.selectOption(o)}
            >
              <span
                class="circle ${l?"circle--selected":""}"
                role="radio"
                aria-labelledby=${n}
  ...
  `]}}customElements.get("esa-radio-group")||customElements.define("esa-radio-group",v);const d=document.getElementById("rg-form");d?.addEventListener("submit",s=>{s.preventDefault();const e=new FormData(d);document.getElementById("rg-out").textContent="priority = "+JSON.stringify(e.get("priority"))});
