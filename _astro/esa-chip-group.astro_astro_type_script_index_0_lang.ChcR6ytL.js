import{i as d,b as n,a as h}from"./lit-element.D8DSg5zn.js";import{t as p}from"./typography.KBHeYOQc.js";import{a as b}from"./a11y.sqk3bMt7.js";const v={xs:"microcopy-2xs-strong",sm:"microcopy-xs-strong",md:"microcopy-md-strong",lg:"microcopy-lg-strong"};class m extends d{constructor(){super(),this.onKeydown=e=>{const t=this.options;if(!t||t.length===0)return;const a=this.renderRoot.querySelectorAll(".chip"),o=Array.from(a).indexOf(this.renderRoot.activeElement),i=this.multiple?Math.max(0,o):Math.max(0,t.findIndex(u=>u.value===this.value));let r;switch(e.key){case"ArrowRight":case"ArrowDown":r=(i+1)%t.length;break;case"ArrowLeft":case"ArrowUp":r=(i-1+t.length)%t.length;break;case"Home":r=0;break;case"End":r=t.length-1;break;case"Enter":case" ":e.preventDefault(),this.select(t[i]);return;default:return}e.preventDefault(),this.multiple||this.select(t[r]),a[r]?.focus()},this.options=[],this.value="",this.values=[],this.multiple=!1,this.size="md",this.label="",this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={options:{type:Array},value:{type:String,reflect:!0},values:{type:Array},multiple:{type:Boolean,reflect:!0},size:{type:String,reflect:!0},name:{type:String,reflect:!0},label:{type:String}}}willUpdate(e){if(e.has("options")&&typeof this.options=="string")try{this.options=JSON.parse(this.options)}catch{this.options=[]}if(e.has("values")&&typeof this.values=="string")try{this.values=JSON.parse(this.values)}catch{this.values=[]}(e.has("value")||e.has("values")||e.has("multiple"))&&this.syncFormValue()}connectedCallback(){super.connectedCallback(),this.internals.role=this.multiple?"group":"radiogroup",this.label&&(this.internals.ariaLabel=this.label),this.syncFormValue()}updated(){this.label&&(this.internals.ariaLabel=this.label)}syncFormValue(){this.multiple?this.internals.setFormValue(this.values.length?this.values.join(","):null):this.internals.setFormValue(this.value||null)}isActive(e){return this.multiple?this.values.includes(e.value):e.value===this.value}select(e){if(this.multiple){this.values=this.values.includes(e.value)?this.values.filter(t=>t!==e.value):[...this.values,e.value],this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{values:[...this.values]},bubbles:!0,composed:!0}));return}e.value!==this.value&&(this.value=e.value,this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0})))}render(){return n`
      <div class="root" @keydown=${this.onKeydown}>
        ${(this.options??[]).map((e,t)=>{const a=this.isActive(e),o=this.multiple?t===0:a;return n`
            <button
              type="button"
              role=${this.multiple?"checkbox":"radio"}
              class="chip chip--${e.tone??"neutral"} ${a?"chip--active":""} typography-${v[this.size]}"
              part="chip"
              tabindex=${o?0:-1}
              aria-checked=${a}
              @click=${()=>this.select(e)}
  ...
  `]}}customElements.get("esa-chip-group")||customElements.define("esa-chip-group",m);const c=[{value:"all",label:"All",tone:"neutral"},{value:"open",label:"Open",tone:"brand"},{value:"flagged",label:"Flagged",tone:"amber"}],g=[{value:"n",label:"Neutral",tone:"neutral"},{value:"ns",label:"Neutral strong",tone:"neutral-strong"},{value:"t",label:"Brand",tone:"brand"},{value:"a",label:"Amber",tone:"amber"}],l=(s,e)=>{const t=document.querySelector(s);t&&(t.options=e)};l('[data-demo="preview"]',c);l('[data-demo="tones"]',g);for(const s of["xs","sm","md","lg"])l(`[data-demo="size-${s}"]`,c);
