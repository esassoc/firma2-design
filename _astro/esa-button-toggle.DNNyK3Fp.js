import{i as l,b as s,a as c}from"./lit-element.D8DSg5zn.js";import{t as d}from"./typography.KBHeYOQc.js";import{a as h}from"./a11y.sqk3bMt7.js";import{o as p}from"./unsafe-svg.XdkOy8tF.js";const u={xs:"label-2xs",sm:"label-xs",md:"label-md",lg:"label-lg"},g={xs:"microcopy-2xs",sm:"microcopy-xs",md:"microcopy-md",lg:"microcopy-lg"},b={xs:"microcopy-2xs-strong",sm:"microcopy-xs-strong",md:"microcopy-md-strong",lg:"microcopy-lg-strong"};class m extends l{constructor(){super(),this.onKeydown=e=>{if(this.disabled)return;const r=this.options;if(r.length===0)return;const a=this.selectedIndex>=0?this.selectedIndex:0;let t;switch(e.key){case"ArrowRight":case"ArrowDown":t=(a+1)%r.length;break;case"ArrowLeft":case"ArrowUp":t=(a-1+r.length)%r.length;break;case"Home":t=0;break;case"End":t=r.length-1;break;case"Enter":case" ":e.preventDefault(),this.select(r[a]);return;default:return}e.preventDefault(),this.select(r[t]),this.focusButton(t)},this.label="",this.helpText="",this.errorText="",this.options=[],this.value="",this.size="md",this.disabled=!1,this.required=!1,this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={label:{type:String},helpText:{type:String,attribute:"help-text"},errorText:{type:String,attribute:"error-text"},options:{type:Array},value:{type:String},size:{type:String,reflect:!0},name:{type:String,reflect:!0},disabled:{type:Boolean,reflect:!0},required:{type:Boolean}}}get resolvedHelpText(){return this.helpText}connectedCallback(){super.connectedCallback(),this.syncFormValue()}willUpdate(e){(e.has("value")||e.has("options"))&&this.syncFormValue()}updated(){this.syncValidity()}syncValidity(){if(!this.required||this.value){this.internals.setValidity({});return}const e=this.renderRoot?.querySelector(".option")??void 0;this.internals.setValidity({valueMissing:!0},this.label?`Select ${this.label}.`:"Select an option.",e)}get selectedIndex(){return this.options.findIndex(e=>e.value===this.value)}get focusIndex(){const e=this.selectedIndex;return e>=0?e:0}syncFormValue(){this.internals.setFormValue(this.value||null)}select(e){this.disabled||e.value===this.value||(this.value=e.value,this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0})))}focusButton(e){this.renderRoot.querySelectorAll(".option")[e]?.focus()}render(){const e=!!this.label,r=!!this.errorText,a=this.resolvedHelpText,t=r?"error":a?"help":null;return s`
      ${e?s`<span class="label typography-${u[this.size]}" id="label">
            ${this.label}${this.required?s`<span class="required" aria-hidden="true">*</span>`:null}
          </span>`:null}
      <div
        class="group ${r?"group--error":""}"
        role="radiogroup"
        aria-labelledby=${e?"label":null}
        aria-required=${this.required?"true":null}
        aria-invalid=${r?"true":null}
        aria-describedby=${t}
        @keydown=${this.onKeydown}
      >
        ${this.options.map((o,n)=>{const i=n===this.selectedIndex;return s`<button
            type="button"
            role="radio"
            class="option ${i?"option--selected":""} typography-${i?b[this.size]:g[this.size]}"
            aria-checked=${i}
          ...
          >
            ...
            ${o.label?s`<span class="option__label">${o.label}</span>`:null}
          </button>`})}
      </div>
      ${r?s`<span class="error typography-body-sm" id="error">${this.errorText}</span>`:a?s`<span class="help typography-body-sm" id="help">${a}</span>`:null}
  ...
  `]}}customElements.get("esa-button-toggle")||customElements.define("esa-button-toggle",m);
